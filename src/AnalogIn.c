...
int ret = PinSetMode(7, true);
    ...
    ret = PinSetMode(0, true);
    printk("PinSetMode returned: %d\n", ret);

    while (1)
    {
        int16_t v = AnalogRead(7);
        int16_t v2 = AnalogRead(0);
...
{
    if (channel < 0 || channel >= AIN_COUNT)
        return -EINVAL;
...
{
    if (channel < 0 || channel >= AIN_COUNT)
        return BAD_ANALOG_READ;
    if (!pin_is_analog[channel])
        return BAD_ANALOG_READ;
...
// digital functions should be moved to luazephyrlib
