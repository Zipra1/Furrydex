...
// this code is from https://github.com/SlimeVR/SlimeVR-Tracker-nRF
// shoutout SlimeVR, i love that project
...
atomic_t battery_percent = ATOMIC_INIT(100);

int update_battery_values()
{
	while (true)
	{
		int battery_mv;
		...
		atomic_set(&battery_percent, battery_pptt / 100);
		k_msleep(1000); // 1 second between checks, should be longer but then would also need a trigger on usb plug/unplug
	}
}

K_THREAD_DEFINE(battery_update_tid, 512, update_battery_values, NULL, NULL, NULL, 10, 0, 0);